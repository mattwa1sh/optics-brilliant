/**
 * Optics Game - Puzzles Module
 *
 * This file contains all the puzzle-related functionality for the Optics Game.
 */

// Puzzle variables
let currentPuzzle = null;
let currentPuzzleFilename = null; 
...
let userPath = [];                       // The reflections/points the user clicked in path identification mode
let expectedPath = [];                   // The correct path from target reflection to ball
let isPathIdentificationMode = false;
let pathHighlightTimeout = null; 
let userRayPath = null; 

// Detective/Investigator puzzle variables
let isInvestigatorMode = false;
let badReflections = [];
let originalPositions = [];              // Original positions of manipulated reflections
let markedAsBad = []; 
let currentUserPath = []; 
let originalReflectionPositions = null;
...
function loadPuzzle(puzzleFilename) {
  ...
  try {
    ...
    currentPuzzleFilename = puzzleFilename; 
    ...
    fetch(`puzzles/${puzzleFilename}`)
      ...
      .then(puzzleData => {
        ...
        const validTypes = ["presentation", "path_identification", "investigator", "generation"];
...
function displayPuzzleInfo() {
  ...
  if (currentPuzzle.type === "investigator") {
    setTimeout(() => {
...
window.canMoveObjects = function() {
  ...
  if (isInvestigatorMode) {
    console.log("Investigator mode - cannot move objects");
    return false;
  }
...
window.isObjectMovable = function(objectType) {
  ...
  // In detective or path identification mode, never allow movement
  if (isInvestigatorMode || isPathIdentificationMode) {
...
// Draw puzzle overlay elements depending on the current mode
function drawPuzzleOverlay() {
  ...
  if (isPuzzleMode) {
    ...
    // For investigator mode, don't skip ray paths
    else if (isInvestigatorMode) {
      // Don't draw investigator overlay here - we'll do it separately
...
// Modified mousePressed handler that should be called from optics-game.js
function handlePuzzleMousePressed() {
  console.log("===== HANDLE PUZZLE MOUSE PRESSED =====");
  console.log("Mouse position:", mouseX, mouseY);
  console.log("isPuzzleMode:", isPuzzleMode);
  console.log("isPathIdentificationMode:", isPathIdentificationMode);
  console.log("isInvestigatorMode:", isInvestigatorMode);
  console.log("isPuzzleSolved:", isPuzzleSolved);
  console.log("isPuzzleFailed:", isPuzzleFailed);
  ...
  if (isPuzzleMode) {
    ...
    // Handle investigator mode clicks
    if (isInvestigatorMode && !isPuzzleSolved && !isPuzzleFailed) {
      console.log("In investigator mode - deferring to double-click handler");
      // For now, just return false - we'll use double-clicks for investigator mode
      return false;
    }
  }
...
// Handle double click events for investigator mode
window.handlePuzzleDoubleClick = function() {
  if (isPuzzleMode && isInvestigatorMode && !isPuzzleSolved && !isPuzzleFailed) {
...
// Reset all global variables related to puzzles
function resetPuzzleVariables() {
  // General puzzle variables
  isPuzzleMode = false;
  currentPuzzle = null;
  isPuzzleSolved = false;
  isPuzzleFailed = false;
  
  // Path identification variables
  isPathIdentificationMode = false;
  pathSolution = null;
  currentUserPath = [];
...
// Setup a detective/investigator puzzle
function setupDetectivePuzzle() {
  console.log("Setting up investigator puzzle");
  
  // Set detective mode flag
  isInvestigatorMode = true;
  
  // Store the original positions of reflections
  // after they're manipulated so we can highlight the manipulated ones later
  originalReflectionPositions = [];
  
  // Ensure manipulatedReflections exists to prevent errors
  if (!currentPuzzle.manipulatedReflections) {
    console.error("No manipulatedReflections array found in the investigator puzzle");
  ...
  // Only set rays visible if not specified in the puzzle config
  if (currentPuzzle.showRaysStart === undefined) {
    console.log("No showRaysStart specified in puzzle, defaulting to false for investigator mode");
    showRayPaths = false;
  } else {
    console.log("Using puzzle's showRaysStart setting:", currentPuzzle.showRaysStart);
    showRayPaths = Boolean(currentPuzzle.showRaysStart);
  }
  
  // Set currentRayIndex if rays should be visible
  if (showRayPaths && reflections.length > 0) {
    currentRayIndex = 0;
  }
  ...
  console.log("Investigator puzzle setup complete");
...
// Submit the detective solution
function submitDetectiveSolution() {
  console.log("Submitting investigator solution");
  
  // Can't submit if we're not in detective mode
  if (!isInvestigatorMode) {
    console.error("Not in investigator mode - cannot submit");
    return;
  }
